...
8. **MJO no Contexto Global e Regional:** Alternância mutuamente exclusiva para cada uma das 8 fases entre:
   ...
   - *Potencial de velocidade 200 hPa (NOAA/PSL):* Compostos contínuos de $\chi_{200}$ da NOAA Physical Sciences Division mostrando convecção ativa / divergência em altos níveis (tons de roxo e azul, $\le -1\times 10^6\text{ m}^2\text{s}^{-1}$) e convecção suprimida / convergência em altos níveis (tons de amarelo, laranja e vermelho, $\ge +1\times 10^6\text{ m}^2\text{s}^{-1}$) ao longo da faixa tropical 30°S–30°N, com escala e projeção simultânea nas visões global e regional.
...
- **NOAA/ESRL Physical Sciences Division (PSL):** *MJO 200-hPa Velocity Potential ($\chi_{200}$) Phase Composites*, NOAA PSL Climate Diagnostics.
...
| **Adição dos compostos de Potencial de Velocidade em 200 hPa da NOAA/PSL sem remover visualizações existentes** | NOAA/ESRL Physical Sciences Division (compostos de $\chi_{200}$ das fases 1–8) | Implementar nova camada mutuamente exclusiva com colorbar contínuo | Adicionado botão `Potencial de velocidade 200 hPa (NOAA/PSL)` preservando `Convecção tropical`, `Trilha 1–8` e `Nenhuma`. Renderiza a onda-1 de $\chi_{200}$ (30°S–30°N) com divergência (ativo, roxo/azul) e convergência (suprimido, amarelo/laranja/vermelho), com barra de escala idêntica à figura oficial da NOAA. |
...
| **Inconsistência entre jato desenhado e narração (extensão/confinamento longitudinal)** | Roy et al. (2025); Hoskins & Ambrizzi (1993) | Alinhar narração com a convenção de traçado contínuo | A narração alegava que o traçado desenhado mostrava extensão a leste no El Niño e confinamento a oeste na La Niña, quando `drawJets` desenhava as mesmas longitudes. A alegação foi removida, registrando que o traçado único é uma referência conceitual de guia de ondas contínuo, com espessura qualitativa modulada pelo ENOS e latitude pelo ciclo sazonal (~32°S em DJF, ~27°S em JJA), explicitadas como convenções didáticas de desenho e não valores climatológicos rígidos observados. |
