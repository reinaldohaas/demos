...
2. **Visão Global como Padrão e Visão Regional Detalhada:**
   - **Global (1200×560 px):** Centrado no Oceano Pacífico, ilustrando o arco completo de teleconexões Índico–Pacífico–América do Sul.
   - **Regional da América do Sul (640×520 px):** Cartografia enriquecida com fronteiras internacionais discretas e limites estaduais brasileiros (RS, SC, PR, SP, MS, MG, RJ, ES, GO, MT, BA) e rótulos de referência territorial sem poluição visual. Sem manchas artificiais de convecção da MJO sobre o Brasil.
3. **Topo Principal Integrado (Mapa e Diagrama RMM Lado a Lado):**
   - O mapa de projeção e o Diagrama RMM ocupam a área de topo (`top-main-stage`), garantindo leitura simultânea da fase/amplitude da MJO e da resposta climática.
   - O Diagrama RMM incorpora o controle deslizante de intensidade/amplitude (A) e o círculo unitário (A = 1) no mesmo card compacto, sem barras avulsas.
4. **Sistema de Painéis Móveis, Redimensionáveis e Organizáveis:**
   - **Arrasto e Movimentação:** Cabeçalho arrastável por mouse e toque em todos os cartões explicativos (RMM, Narração, Resultado/Fonte, PSA, TSM e Fase Selecionada).
   - **Controles de Janela:** Ações de recolher/minimizar (`_`) e fechar/ocultar (`×`) em cada cabeçalho.
   - **Menu "Painéis ▼":** Dropdown acessível para reabrir painéis fechados ou alternar visibilidade a qualquer momento.
   - **Restaurar Layout:** Botão "Restaurar layout" para retornar à disposição original padrão.
   - **Persistência Local:** Coordenadas e estados salvos no `localStorage` do navegador.
   - **Confinamento Seguro:** Travamento nas bordas da tela (*viewport clamping*), impedindo que caixas fiquem inacessíveis fora da janela.
   - **Responsividade e Teclado:** Em telas estreitas (≤ 960 px), os painéis empilham-se sequencialmente sem exigir arraste. Foco de teclado permite recolher com Enter/Espaço e reposicionar com teclas direcionais. Totalmente desacoplado do estado climático (organizar a interface nunca altera seleções meteorológicas).
5. **Separação Rigorosa de Variáveis:** Camadas independentes para *Extremos de chuva* e *Chuva média*. Extremos de precipitação não são inferidos como risco de granizo ou tornados.
6. **Sem Parâmetros ou Cálculos Fictícios:** Sem geradores combinatórios livres, sem probabilidades sintéticas e sem percentuais artificiais.
7. **TSM Equatorial Qualitativa:** Representação do sinal no Pacífico equatorial central/leste (El Niño quente, La Niña fria e Neutro próximo à média), claramente identificada como qualitativa.
8. **MJO no Contexto Global e Regional:** Alternância mutuamente exclusiva para cada uma das 8 fases entre:
   - *Convecção tropical (esquema RMM):* Compostos conceituais de anomalias ativas [+] e suprimidas [−] baseados no padrão climatológico Wheeler & Hendon (2004) e CPC/NOAA, restritos às bacias oceânicas equatoriais na visão global.
   - *Divergência/convergência em altitude (esquema conceitual):* Ilustração qualitativa didática da relação de Poisson e do vento divergente ($\vec{v}_\chi = \nabla\chi$, $\nabla^2\chi = -\delta$) em ~200 hPa, ilustrando divergência anômala (convecção ativa, tons de roxo/azul) e convergência anômala (convecção suprimida, tons de amarelo/laranja/vermelho) ao longo de 30°S–30°N. Restrita à visão global, sem escala numérica falsa (eliminadas unidades de $10^6\text{ m}^2\text{s}^{-1}$ de −5 a +5 e referência indevida à NOAA/PSL) e sem projeção de manchas sobre o Brasil na visão regional.
   - *Trilha 1–8:* Marcações nominais ao longo do equador com fase ativa destacada e números interativos clicáveis.
   - *Nenhuma:* Sem exibição de camadas tropicais da MJO para a fase.
   Ao ligar qualquer uma das opções, as outras são desligadas automaticamente.
9. **Teleconexão PSA Não Permanente:** Corredor conceitual de propagação de ondas de Rossby traçado exclusivamente nas combinações com mecanismo verificado na literatura, sem centros A/B fictícios.
10. **Jatos e Desobstrução Espacial (Base Visual Permanente):** Jato Subtropical (~200 hPa) em traçado único com espessura representando a modulação pelo ENOS e posição norte–sul acompanhando a estação; SALLJ (~850 hPa) em seta única canalizada a leste dos Andes em direção ao SESA (Jones et al. 2023). Símbolo de chuva, SALLJ, Jato Subtropical e rótulos posicionados sem colisões visuais.
11. **Rótulo Geográfico Rigoroso:** Mantido estritamente "SESA" no topo da região, sem "Bacia do Prata" no rótulo e sem "Andes (Bloqueio Orográfico)" no mapa.
12. **Legenda Inicialmente Oculta:** Exibida e recolhida por botão de alternância acessível com ARIA.
13. **Narração Completa & Acessibilidade:** Toda combinação selecionável possui texto explicativo gerado na tela e síntese de voz disponível (com controles de narrar, pausar/retomar, parar e silenciar). Ao alterar controles com a fala ativa, falas anteriores são canceladas e a nova combinação é narrada automaticamente. O glossário foi removido desta demonstração conforme instrução.
...
### Matriz Comparativa da Auditoria Científica e de Interface:

| Problema | Passagem/Figura Consultada | Decisão | Correção Realizada |
| :--- | :--- | :--- | :--- |
| **Camada indevidamente apresentada como dados da NOAA/PSL com escala física não rastreável** | Helmholtz / Poisson ($\vec{v}_\chi = \nabla\chi$, $\nabla^2\chi = -\delta$); Princípio de transparência metodológica | Retirar identificação NOAA/PSL e escala numérica; apresentar como esquema conceitual qualitativo | Removida qualquer atribuição a NOAA/PSL e escalas numéricas de −5 a +5 em $10^6\text{ m}^2\text{s}^{-1}$. Apresentado expressamente como esquema conceitual de divergência/convergência em ~200 hPa ($\nabla^2\chi = -\delta$), restrito à visão global e sem inferir chuva local sobre o Brasil nem desenhar manchas na visão regional. |
| **Atalhos restritos a DJF ("Evolução em DJF") sem contemplar as 4 estações** | Roy et al. (2025), Seção 3 e Figs. 2–3; Fernandes & Grimm (2023) | Renomear para "Eventos de interesse" e estruturar atalhos para DJF, MAM, JJA e SON | Substituída a barra de atalhos por "Eventos de interesse" com seções para as 4 estações. Cadastrados os casos de MAM (pares 4–5 e 6–7), JJA (par 2–3) e SON (par 4–5). Mantida a regra física de que estudos de circulação hemisférica e geopotencial ($Z_{200}$) não avaliam precipitação local em SESA/ZCAS (`mean: null`, `extremes: null`), evitando símbolos espúrios de chuva. |
| **Fases agrupadas em pares (2–3, 4–5, 6–7, 8–1) sem identificação clara** | Requisito de transparência documental metodológica | Identificar explicitamente a agregação de fases no título, resumo, resultado e narração | Incluída nota obrigatória: *"Evidência para fases agrupadas X–Y; não é um resultado exclusivo da fase selecionada"*. Selecionar atalhos preserva integralmente a amplitude definida pelo usuário, sem alteração automática. |
| **Desalinhamento angular de 22,5° nos cliques dos setores do diagrama RMM** | Geometria analítica e trigonometria do diagrama RMM Wheeler & Hendon (2004) | Alinhar as faixas angulares às linhas divisórias a cada 45° | Substituídas as fronteiras deslocadas pelos limites geométricos dos octantes ($[0,45) \to 5$, $[45,90) \to 6$, $[90,135) \to 7$, $[135,180] \to 8$, $[-180,-135) \to 1$, $[-135,-90) \to 2$, $[-90,-45) \to 3$, $[-45,0) \to 4$), garantindo 100% de precisão nos cliques. |
| **Cache persistente em navegadores de arquivos estáticos do GitHub Pages** | Especificação HTTP de Cache Invalidation | Adicionar versionamento por query string e meta tags anti-cache | Adicionado parâmetro `?v=20260925e` a todas as tags `<script>` e inseridas meta tags `Cache-Control: no-cache, no-store, must-revalidate` em `index.html`. |
| **Mapa não aparecendo sob nenhuma circunstância** | Diagnóstico de runtime do DOM (`update()` e `$('psaCard')`) | Corrigir referência segura a `panelPsa` e `psaCard` | Resolvida a exceção que abortava a execução de `update()` antes da chamada a `drawMap()`, garantindo que o mapa seja renderizado imediatamente com todos os seus 400+ elementos gráficos. |
| **Diagrama RMM e barra de amplitude separados e dispersos** | Requisito de usabilidade e Wheeler & Hendon (2004) | Integrar RMM e controle de intensidade no topo ao lado do Mapa | Posicionado o card compacto RMM no topo (`top-main-stage`) ao lado direito do mapa de projeção, com o controle de amplitude diretamente integrado ao card abaixo do círculo unitário. |
| **Interface rígida sem mobilidade de painéis explicativos** | Requisito de organização modular da interface | Implementar gerenciador leve e desacoplado de painéis móveis | Criado `panels-manager.js` com suporte a arrastar por cabeçalho, redimensionar, recolher (`_`), fechar (`×`), menu "Painéis ▼", "Restaurar layout", persistência em `localStorage`, confinamento na tela e fallback responsivo. Totalmente desacoplado dos dados climáticos. |
| **Leitura da América do Sul e do Brasil sem referências territoriais discretas** | Cartografia de mesoescala e divisão federativa do Brasil | Adicionar limites estaduais discretos e fronteiras internacionais na visão regional | Inseridos traçados vetoriais discretos para os países vizinhos e estados brasileiros (RS, SC, PR, SP, MS, MG, RJ, ES, GO, MT, BA) e rótulos geográficos sem interferir na visualização de jatos e chuva. |
| **Manchas conceituais da MJO sobrepostas artificialmente ao Brasil** | Wheeler & Hendon (2004); CPC/NOAA | Restringir convecção tropical às bacias oceânicas na visão global | Removidas manchas artificiais que cruzavam a América do Sul; dipolos tropicais de convecção/supressão mantidos estritamente na faixa equatorial oceânica e restritos à visão global. |
| **Sobreposição entre jatos, SALLJ, SESA e símbolos de chuva** | Princípios de clareza gráfica cartográfica | Reposicionar traçados e rótulos com distanciamento vertical | Afastado o rótulo "SESA" para o topo do polígono (24°S), posicionado o SALLJ entre 59°W e 57°W, e ajustado o ícone de chuva para 33°S, eliminando colisões de traçado. |
| **ENOS Neutro 1 descrito como "declínio" e ZCAS inteira destacada em 8–1** | Fernandes & Grimm (2023), Seções 4.b e 5, Figs. 4 e 12 | Corrigir caracterização de circulação e retirar destaque na ZCAS inteira | Esclarecido que a teleconexão em circulação e anomalia anticiclônica no Atlântico subtropical estão mais claramente estabelecidas na fase 1 do que na fase 8. Em precipitação, as anomalias são mais fracas ou não significativas em partes da ZCAS, deslocadas para o centro de CESA e norte. Removido o destaque generalizado na ZCAS (`mean: null` em 8 e 1; extremos focados em CESA na fase 8). |
| **Afirmação de "teleconexões intrassazonais enfraquecidas no inverno" atribuída a Roy et al.** | Roy et al. (2025), Seção 3 e Figs. 2–3 | Corrigir afirmação sazonal e evitar generalizações regionais | Roy et al. (2025) demonstram que as teleconexões extratropicais no Hemisfério Sul são amplificadas sob condições de ENOS no outono (MAM) e inverno (JJA). A narração foi revisada para contextualizar a posição média do jato no inverno (~27°S) e a amplificação extratropical por ENOS documentada pelo estudo, esclarecendo a ausência de suporte nesta síntese para inferência direta de chuva no SESA em JJA. |
| **Posicionamento latitudinal do domínio SESA** | Climatologia regional do SESA e bacias hidrográficas | Deslocar o polígono SESA mais ao sul | Ajustadas as coordenadas de `SESA_POLY` em `map-regions.js` de 22°S–37°S para 25°S–40°S, posicionando o domínio estritamente no Sul do Brasil (PR, SC, RS), Uruguai e Nordeste da Argentina. Ajustados o rótulo "SESA" para latitude 24°S e o ícone de chuva para 33°S. |
| **Relatório de testes alegando "integridade científica comprovada"** | Padrões de teste e CI (Node.js/DOM mocks) | Substituir por relatório fiel dos testes executados | Substituída a mensagem por um relatório estritamente técnico: integridade estrutural dos dados, 96 combinações básicas testadas em 384 estados, ausência de símbolos em combinações não cadastradas (null), permanência de ZCAS e SESA, geração de texto acessível e testes de máquina de estados de voz no mock. O relatório registra expressamente que testes de software não constituem validação bibliográfica de conteúdo. |
...
2. **El Niño:** A piscina quente deslocada para leste altera o estado básico e o guia de ondas do Jato Subtropical; a convecção-fonte subtropical ocorre **deslocada para leste nas fases 8–1**. Com a defasagem dinâmica de propagação de ondas, a **resposta destacada na ZCAS e o maior aumento de extremos no CESA (ao norte de 15°S) manifestam-se na fase 1**. Em contrapartida, na fase 3, a forçante no Índico tropical interage construtivamente com o El Niño para gerar o maior aumento de extremos no SESA.
3. **ENOS Neutro:** Em circulação, o trem de ondas PSA e a anomalia anticiclônica no Atlântico subtropical atingem maior coerência e definição espacial na fase 1 em comparação com a fase 8. Em precipitação, as anomalias positivas são mais fracas ou não significativas em setores da ZCAS, deslocadas para o centro de CESA e norte, sem justificar destaque para a ZCAS inteira em nenhuma das duas fases. Na fase 8, registra-se aumento de extremos localizado no CESA (ao norte de 15°S). Para o SESA, a forçante no Índico e Continente Marítimo nas fases 3–4 produz o **maior aumento da frequência de extremos no SESA na fase 4** (fase 3 precursora), associado ao transporte meridional de umidade amazônica (SALLJ tipo Central de Jones et al. 2023) e cavado no Atlântico Sudoeste.
...
- **Teste de Verificação Automatizado:** `node verify-model.cjs` (100% aprovado para execução funcional, ausência de destaques espúrios e máquina de estados de voz).
